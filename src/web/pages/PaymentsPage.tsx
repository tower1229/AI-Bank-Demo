...
import { BackLink, CustomerSearchTable, SectionHeader, TextInput } from "../components/common";
import { useConfirm } from "../hooks/useConfirm";
import { postJson } from "../lib/api";
import { formatUsd, usdToCents } from "../lib/format";
import type { RunAction } from "../types";

export function PaymentsPage({ customers }: { customers: CustomerSearchResult[] }) {
  return (
    <article className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <SectionHeader title="Payment Instructions" detail="Review funding accounts before booking an internal USD payment." />
...
export function PaymentInstructionPage({ runAction }: { runAction: RunAction }) {
  ...
  return (
    <article className="max-w-3xl rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      <BackLink to="/payments" label="Back to payment instructions" />
