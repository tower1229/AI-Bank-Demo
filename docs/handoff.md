...
- Enhanced onboarding flow: Telegram/OpenClaw mixed document-image/text intake, address proof capture, KYC evidence image, KYC review package, and a web-console onboarding detail review page.
...
- Submitted onboarding applications include structured KYC checks and review reasons for workflow realism. This is not real external KYC, AML, sanctions, tax, PEP, or suitability screening.
...
- The intended onboarding behavior is now mixed intake: merge image-parsed passport and address proof fields with text-supplied business fields, request one KYC evidence image, show captured/missing/correction-needed status, then show one final summary before calling MCP.
...
- Onboarding review now has a dedicated detail route at `/client-lifecycle/:applicationId`, including applicant profile, identity document fields, address proof fields, KYC checklist, review reasons, source metadata, and web-only approval.
...
1. Complete full visual browser QA against local Wrangler dev.
   ...
   - Onboarding detail review page and KYC checklist.
